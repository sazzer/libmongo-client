...
test_bson_generate_flat (void)
{
  ...
  bson_append_boolean (b, "FALSE", FALSE);
  bson_append_string (b, "goodbye", "cruel world, this garbage is gone.",
		      strlen ("cruel world"));
  bson_append_utc_datetime (b, "date", 1294860709000);
  bson_append_double (b, "double", 3.14);
  bson_append_int64 (b, "int64", 9876543210);
  bson_append_null (b, "null");
  bson_append_boolean (b, "TRUE", TRUE);
  bson_append_string (b, "hello", "world", -1);
